...
var minimist = require('minimist'),
    ...
    help = [
      'Options:',
      '  --file          The absolute path of the script to be run as a process.                                               [required]',
      '  --log           The absolute path of the log file.                                                                    [required]',
      '  --errorlog      The absolute path of the error log file.                                                              [required]',
      '  --title         The name/title of the process.                                                                        [required]',
      '  --maxretries    The maximim number of times the process will be auto-restarted.                                       [default: -1]',
      '  --maxrestarts   The maximim number of times the process should be restarted within a 60 second period shutting down.  [default: 5]',
      '  --wait          The number of seconds between each restart attempt.                                                   [default: 1]',
      '  --grow          A percentage growth rate at which the wait time is increased.                                         [default: 0.25]',
      '  --abortonerror  Do not attempt to restart the process if it fails with an error,                                      [default: "no"]'
    ].join('\n');

    var missing = [];
    if (!("file" in argv)) missing.push('file');
    if (!("log" in argv)) missing.push('log');
    if (!("errorlog" in argv)) missing.push('errorlog');
    if (!("title" in argv)) missing.push('title');
